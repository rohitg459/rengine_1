...
def initiate_subtask(
    ...
    try:
        yaml_configuration = yaml.load(
            engine.yaml_configuration, Loader=yaml.FullLoader
        )
...
def initiate_scan(
    ...
    task.fetch_url = False
    task.osint = False
    task.screenshot = False
    task.vulnerability_scan = False
    ...
    try:
        current_scan_dir = (
            domain.name + "_" + str(random.randint(100000000000, 999999999999))
        )
    ...
    http_crawler(task, domain, yaml_configuration, results_dir, activity_id)
    update_last_activity(activity_id, 2)
...
def subdomain_scan(
    ...
    if ALL in yaml_configuration[SUBDOMAIN_DISCOVERY][USES_TOOLS]:
        ...
        tools = " ".join(
            str(tool).lower()
            for tool in yaml_configuration[SUBDOMAIN_DISCOVERY][USES_TOOLS]
        )
        ...
        for tool in tools.split(" "):
            ...
            if tool in tools:
                if tool == "amass-passive":
                    amass_command = (
                        "amass enum -passive -d {} -o {}/from_amass.txt".format(
                    ...
                    if AMASS_WORDLIST in yaml_configuration[SUBDOMAIN_DISCOVERY]:
                        wordlist = yaml_configuration[SUBDOMAIN_DISCOVERY][
                            AMASS_WORDLIST
                        ]
                        if wordlist == "default":
                            wordlist_path = (
                        ...
                        amass_command = amass_command + " -brute -w {}".format(
                            wordlist_path
                        )
                    ...
                    subfinder_command = (
                        "subfinder -d {} -t {} -o {}/from_subfinder.txt".format(
                            domain.name, threads, results_dir
                        )
                    )
                    ...
                    if (
                        ...
                        and yaml_configuration[SUBDOMAIN_DISCOVERY][
                    ...
                    os.system(
                        "rm -rf /usr/src/github/OneForAll/results/{}.*".format(
                            domain.name
                        )
                    )
                ...
                if InstalledExternalTool.objects.filter(
                    name__icontains=tool.lower()
                ).exists():
                    custom_tool = InstalledExternalTool.objects.get(
                        name__icontains=tool
                    )
                    execution_command = custom_tool.subdomain_gathering_command
                    print(execution_command)
                    # replace syntax with actual commands and path
                    if (
                        "{TARGET}" in execution_command
                        and "{OUTPUT}" in execution_command
                    ):
                        execution_command = execution_command.replace(
                            "{TARGET}", domain.name
                        )
                        execution_command = execution_command.replace(
                            "{OUTPUT}", "{}/from_{}.txt".format(results_dir, tool)
                        )
                        execution_command = (
                            execution_command.replace(
                                "{PATH}", custom_tool.github_clone_path
                            )
                            if "{PATH}" in execution_command
                            else execution_command
                        )
                        logger.info(
                            "Custom tool {} running with command {}".format(
                        ...
                        logger.error(
                            "Sorry can not run this tool! because TARGET and OUTPUT are not available!"
                        )
    except Exception as e:
        logger.error(e)
    ...
    if notification and notification[0].send_subdomain_changes_notif:
        ...
        if removed_subdomain:
            message = "**{} Subdomains are no longer available on domain {}**".format(
...
def http_crawler(
    task, domain, yaml_configuration, results_dir, activity_id, threads=100
):
    """
    ...
    subdomain_scan_results_file = results_dir + "/sorted_subdomain_collection.txt"
    httpx_command = "/go/bin/httpx -status-code -content-length -title -tech-detect -cdn -ip -follow-host-redirects -random-agent -t {}".format(
        threads
    )
    ...
    httpx_command += " -json -o {} -l {}".format(
        httpx_results_file, subdomain_scan_results_file
    )
                ...
                if "a" in json_st:
                    for _ip in json_st["a"]:
                        ...
                        subprocess_output = subprocess.getoutput(
                            ["geoiplookup {}".format(_ip)]
                        )
                        if (
                            "IP Address not found" not in subprocess_output
                            and "can't resolve hostname" not in subprocess_output
                        ):
                            country_iso = (
                                subprocess_output.split(":")[1].strip().split(",")[0]
                            )
                            country_name = (
                                subprocess_output.split(":")[1]
                                .strip()
                                .split(",")[1]
                                .strip()
                            )
...
def grab_screenshot(task, domain, yaml_configuration, results_dir, activity_id):
    ...
    if (
        SCREENSHOT in yaml_configuration
...
def port_scanning(
    ...
    try:
        ...
        for line in lines:
            ...
            if not Subdomain.objects.filter(
                name=host, scan_history=scan_history, ip_addresses__address=ip_address
            ).exists():
...
def check_waf(scan_history, results_dir):
    ...
    if os.path.isfile(alive_file):
        wafw00f_command = "wafw00f -i {} -o {}".format(alive_file, output_file_name)
                ...
                for line in lines:
                    ...
                    if waf_name != "None":
                        if Waf.objects.filter(
                            name=waf_name, manufacturer=waf_manufacturer
                        ).exists():
                            waf_obj = Waf.objects.get(
                                name=waf_name, manufacturer=waf_manufacturer
                            )
                        else:
                            waf_obj = Waf(name=waf_name, manufacturer=waf_manufacturer)
                            waf_obj.save()
...
def directory_fuzz(
    ...
    if notification and notification[0].send_scan_status_notif:
        send_notification(
            "Directory Bruteforce has been initiated for {}.".format(domain_name)
        )

    # get wordlist
    if (
        WORDLIST not in yaml_configuration[DIR_FILE_FUZZ]
        or not yaml_configuration[DIR_FILE_FUZZ][WORDLIST]
        or "default" in yaml_configuration[DIR_FILE_FUZZ][WORDLIST]
    ):
        wordlist_location = "/usr/src/wordlist/dicc.txt"
    else:
        wordlist_location = (
            "/usr/src/wordlist/" + yaml_configuration[DIR_FILE_FUZZ][WORDLIST] + ".txt"
        )
    ...
    if domain:
        ...
        subdomains_fuzz = Subdomain.objects.filter(name=subdomain).filter(
            scan_history__id=scan_history.id
        )

    if (
        USE_EXTENSIONS in yaml_configuration[DIR_FILE_FUZZ]
        and yaml_configuration[DIR_FILE_FUZZ][USE_EXTENSIONS]
    ):
        if EXTENSIONS in yaml_configuration[DIR_FILE_FUZZ]:
            extensions = ",".join(
                "." + str(ext) for ext in yaml_configuration[DIR_FILE_FUZZ][EXTENSIONS]
            )

            ffuf_command = " {} -e {} ".format(ffuf_command, extensions)

    if (
        THREADS in yaml_configuration[DIR_FILE_FUZZ]
        and yaml_configuration[DIR_FILE_FUZZ][THREADS] > 0
    ):
        threads = yaml_configuration[DIR_FILE_FUZZ][THREADS]
        ffuf_command = " {} -t {} ".format(ffuf_command, threads)

    if (
        RECURSIVE in yaml_configuration[DIR_FILE_FUZZ]
        ...
        ffuf_command = " {} -recursion -recursion-depth {} ".format(
            ffuf_command, recursive_level
        )

    if (
        STOP_ON_ERROR in yaml_configuration[DIR_FILE_FUZZ]
        and yaml_configuration[DIR_FILE_FUZZ][STOP_ON_ERROR]
    ):
        ffuf_command = "{} -se".format(ffuf_command)

    if (
        FOLLOW_REDIRECT in yaml_configuration[DIR_FILE_FUZZ]
        and yaml_configuration[DIR_FILE_FUZZ][FOLLOW_REDIRECT]
    ):
        ffuf_command = " {} -fr ".format(ffuf_command)

    if (
        AUTO_CALIBRATION in yaml_configuration[DIR_FILE_FUZZ]
        and yaml_configuration[DIR_FILE_FUZZ][AUTO_CALIBRATION]
    ):
        ffuf_command = " {} -ac ".format(ffuf_command)

    if (
        TIMEOUT in yaml_configuration[DIR_FILE_FUZZ]
        and yaml_configuration[DIR_FILE_FUZZ][TIMEOUT] > 0
    ):
        timeout = yaml_configuration[DIR_FILE_FUZZ][TIMEOUT]
        ffuf_command = " {} -timeout {} ".format(ffuf_command, timeout)

    if (
        DELAY in yaml_configuration[DIR_FILE_FUZZ]
        and yaml_configuration[DIR_FILE_FUZZ][DELAY] > 0
    ):
        delay = yaml_configuration[DIR_FILE_FUZZ][DELAY]
        ffuf_command = ' {} -p "{}" '.format(ffuf_command, delay)

    if MATCH_HTTP_STATUS in yaml_configuration[DIR_FILE_FUZZ]:
        mc = ",".join(
            str(code) for code in yaml_configuration[DIR_FILE_FUZZ][MATCH_HTTP_STATUS]
        )
    else:
        mc = "200,204"

    ffuf_command = " {} -mc {} ".format(ffuf_command, mc)

    if (
        MAX_TIME in yaml_configuration[DIR_FILE_FUZZ]
        and yaml_configuration[DIR_FILE_FUZZ][MAX_TIME] > 0
    ):
        max_time = yaml_configuration[DIR_FILE_FUZZ][MAX_TIME]
        ffuf_command = " {} -maxtime {} ".format(ffuf_command, max_time)
    ...
    for subdomain in subdomains_fuzz:
        ...
        if subdomain.http_url:
            http_url = (
                subdomain.http_url + "FUZZ"
                if subdomain.http_url[-1:] == "/"
                else subdomain.http_url + "/FUZZ"
            )
        else:
            http_url = subdomain
        ...
        proxy = get_random_proxy()
        if proxy:
            ffuf_command = "{} -x {} ".format(ffuf_command, proxy)

        command = "{} -u {} -o {} -of json".format(ffuf_command, http_url, dirs_results)
    ...
    if notification and notification[0].send_scan_status_notif:
        send_notification(
            "Directory Bruteforce has been completed for {}.".format(domain_name)
        )
...
def fetch_endpoints(
    ...
    if notification and notification[0].send_scan_status_notif:
        send_notification(
            "reNgine is currently gathering endpoints for {}.".format(domain_name)
        )
    ...
    for tool in tools.split(" "):
        ...
        if tool == "gauplus":
            ...
            gauplus_command = (
                "{} | gauplus --random-agent | grep -Eo {} > {}/urls_gau.txt".format(
                    input_target, valid_url_of_domain_regex, results_dir
                )
            )
            ...
            hakrawler_command = (
                "{} | hakrawler -subs -u | grep -Eo {} > {}/urls_hakrawler.txt".format(
                    input_target, valid_url_of_domain_regex, results_dir
                )
            )
            ...
            waybackurls_command = (
                "{} | waybackurls | grep -Eo {} > {}/urls_waybackurls.txt".format(
                    input_target, valid_url_of_domain_regex, results_dir
                )
            )
            ...
            gospider_command += " --js -t 100 -d 2 --sitemap --robots -w -r | grep -Eo {} > {}/urls_gospider.txt".format(
                valid_url_of_domain_regex, results_dir
            )
    ...
    os.system(
        "sort -u {0}/final_urls.txt -o {0}/{1}".format(results_dir, output_file_name)
    )
    ...
    if IGNORE_FILE_EXTENSION in yaml_configuration[FETCH_URL]:
        ...
        os.system(
            "rm {0}/{1} && mv {0}/temp_urls.txt {0}/{1}".format(
        ...
        with open(endpoint_final_url) as endpoint_list:
            for url in endpoint_list:
                ...
                if not EndPoint.objects.filter(
                    ...
                    if (
                        Subdomain.objects.filter(scan_history=scan_history)
    ...
    httpx_command = "/go/bin/httpx -l {0}/{1} -status-code -content-length -ip -cdn -title -tech-detect -json -follow-redirects -random-agent -o {0}/final_httpx_urls.json".format(
        results_dir, output_file_name
    )
    ...
    try:
        if os.path.isfile(url_results_file):
            ...
            for line in lines:
                ...
                if (
                    EndPoint.objects.filter(scan_history=scan_history)
    ...
    if notification and notification[0].send_scan_status_notif:
        endpoint_count = (
            EndPoint.objects.filter(scan_history__id=scan_history.id)
            .values("http_url")
            .distinct()
            .count()
        )
        endpoint_alive_count = (
            EndPoint.objects.filter(
                scan_history__id=scan_history.id, http_status__exact=200
            )
            .values("http_url")
            .distinct()
            .count()
        ...
        send_notification(
            "reNgine has finished gathering endpoints for {} and has discovered *{}* unique endpoints.\n\n{} of those endpoints reported HTTP status 200.".format(
                domain_name, endpoint_count, endpoint_alive_count
            )
        )
...
def vulnerability_scan(
    ...
    if notification and notification[0].send_scan_status_notif:
        if domain:
            ...
            send_notification(
                "Vulnerability scan has been initiated for {}.".format(subdomain.name)
            )
    """
    ...
    if domain:
        ...
        url_to_scan = (
            subdomain.http_url if subdomain.http_url else "https://" + subdomain.name
        )
        nuclei_command = "nuclei -j -u {} -o {}".format(
                ...
                for line in lines:
                    ...
                    try:
                        ...
                        if "matched-at" in json_st:
                            ...
                            if (
                                not EndPoint.objects.filter(scan_history=scan_history)
                                .filter(target_domain=domain)
                                .filter(http_url=json_st["matched-at"])
                                .exists()
                            ):
                                logger.info("Creating Endpoint...")
                                endpoint_dict = DottedDict(
                                ...
                                save_endpoint(endpoint_dict)
                                logger.info(
                                    "Endpoint {} created!".format(json_st["matched-at"])
                                )
                        ...
                        if (
                            "classification" in json_st["info"]
                            and "cve-id" in json_st["info"]["classification"]
                            and json_st["info"]["classification"]["cve-id"]
                        ):
                        ...
                        if (
                            "classification" in json_st["info"]
                            and "cwe-id" in json_st["info"]["classification"]
                            and json_st["info"]["classification"]["cwe-id"]
                        ):
                        ...
                        if "classification" in json_st["info"]:
                            if "cvss-metrics" in json_st["info"]["classification"]:
                                vulnerability.cvss_metrics = json_st["info"][
                                    "classification"
                                ]["cvss-metrics"]
                            if "cvss-score" in json_st["info"]["classification"]:
                                vulnerability.cvss_score = json_st["info"][
                                    "classification"
                                ]["cvss-score"]

                        if (
                            "reference" in json_st["info"]
                            and json_st["info"]["reference"]
                        ):
                            for ref_url in json_st["info"]["reference"]:
                                if VulnerabilityReference.objects.filter(
                                    url=ref_url
                                ).exists():
                                    reference = VulnerabilityReference.objects.get(
                                        url=ref_url
                                    )
                                else:
    ...
    if notification and notification[0].send_scan_status_notif:
        info_count = Vulnerability.objects.filter(
            scan_history__id=scan_history.id, severity=0
        ).count()
        low_count = Vulnerability.objects.filter(
            scan_history__id=scan_history.id, severity=1
        ).count()
        medium_count = Vulnerability.objects.filter(
            scan_history__id=scan_history.id, severity=2
        ).count()
        high_count = Vulnerability.objects.filter(
            scan_history__id=scan_history.id, severity=3
        ).count()
        critical_count = Vulnerability.objects.filter(
            scan_history__id=scan_history.id, severity=4
        ).count()
...
def update_last_activity(id, activity_status, error_message=None):
    ScanActivity.objects.filter(id=id).update(
        status=activity_status, error_message=error_message, time=timezone.now()
    )
