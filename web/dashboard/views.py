...
def index(request):
    ...
    alive_count = Subdomain.objects.all().exclude(http_status__exact=0).count()
    endpoint_alive_count = EndPoint.objects.filter(http_status__exact=200).count()
    ...
    vulnerability_feed = Vulnerability.objects.all().order_by("-discovered_date")[:20]
    ...
    count_subdomains_by_date = (
        Subdomain.objects.filter(discovered_date__gte=last_week)
    ...
    count_vulns_by_date = (
        Vulnerability.objects.filter(discovered_date__gte=last_week)
        .annotate(date=TruncDay("discovered_date"))
        .values("date")
        .annotate(count=Count("id"))
        .order_by("-date")
    ...
    count_scans_by_date = (
        ScanHistory.objects.filter(start_scan_date__gte=last_week)
    ...
    count_endpoints_by_date = (
        EndPoint.objects.filter(discovered_date__gte=last_week)
        .annotate(date=TruncDay("discovered_date"))
        .values("date")
        .annotate(count=Count("id"))
        .order_by("-date")
    ...
    context["most_used_port"] = Port.objects.annotate(count=Count("ports")).order_by(
        "-count"
    )[:7]
    context["most_used_ip"] = (
        IpAddress.objects.annotate(count=Count("ip_addresses"))
        .order_by("-count")
        .exclude(ip_addresses__isnull=True)[:7]
    )
    context["most_used_tech"] = Technology.objects.annotate(
        count=Count("technologies")
    ).order_by("-count")[:7]

    context["most_common_cve"] = (
        CveId.objects.annotate(nused=Count("cve_ids"))
        .order_by("-nused")
        .values("name", "nused")[:7]
    )
    context["most_common_cwe"] = (
        CweId.objects.annotate(nused=Count("cwe_ids"))
        .order_by("-nused")
        .values("name", "nused")[:7]
    )
    context["most_common_tags"] = (
        VulnerabilityTags.objects.annotate(nused=Count("vuln_tags"))
        .order_by("-nused")
        .values("name", "nused")[:7]
    )

    context["asset_countries"] = CountryISO.objects.annotate(
...
def on_user_logged_out(sender, request, **kwargs):
    messages.add_message(
        request,
        messages.INFO,
        "You have been successfully logged out. Thank you " + "for using reNgine.",
    )
